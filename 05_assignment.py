...
def exercise13(n):
    ...
    """
    # ------ Place code below here \/ \/ \/ ------
    dates = pd.date_range(start="1/1/2010", periods=n)
    integers = [x for x in range(0, n)]

    s = pd.Series(integers, index=dates)
    s.cumsum().plot()
    # ------ Place code above here /\ /\ /\ ------
    return s
...
def exercise15():
    ...
    """
    # ------ Place code below here \/ \/ \/ ------
    df = pd.read_csv("https://tinyurl.com/y63q7okz").iloc[::5, [0, 2]]
